...
#Running python code from workspace_utils due to load issues.
import signal
...
for i in range(len(all_pickles)):
    ...
    training_loss[model_names[i]]=train_loss[i][-1]
...
for i in range(len(all_pickles)):
    ...
    val_loss[model_names[i]]=valid_loss[i][-1]
...
for n, i in enumerate(training_loss):
    print("{} training loss: {}, validation loss: {} difference {}".format(i, training_loss[i], val_loss[i], training_loss[i] - val_loss[i]))
    if n > 3: break
...
# 
# __Answer:__ Model_3 Is the best.
# - __Model_0:__  had a very high training and validation loss indicating that it is underfitting.
# - __Model_1:__ added a Batch Normalization and Time Distributed Dense layer which was able improve performance significantly. Overall this perfromed very well. However, in the validation loss graph, it couldn't achieve the lowest score.
# - __Model_2__ Model_2 also perfomed well but similar to model 1, it couldn't achieve a lower validation loss.
# - __Model_3__ Model 3 had the lowest validation loss with relatively close scores with it's training loss indicating it is generalizing well as indicated in the article here: https://towardsdatascience.com/rnn-training-tips-and-tricks-2bf687e67527 .
# - __Model_4__  Although the difference between training and validation were close, this model, much like model 1 & 2 had a higher overall validation loss after 20 epochs.
...
# 
# __Answer:__  This model is based on model 3 which had the most promising performance.
# - To get a a more complicated model, the recuring layers were increased to 3 from 2.
# - Each recurrent layer was made bidirectional so the model can predict audio based on the prior and post inputs. Since language data is heavily contextual with it's surrounding words, bidirectional models tend to be a popular options in these models. Like the machine translation project, bidriectional layers were added in this model.
# - After each recurrent layer a batch normalization was added as it was in model 3.
# - After batch normalization, a dropout rate was added. According to the papers recomended for dropouts, deep models have a tendency of overfitting. Therefore dropout rates are added to balance this out. A dropout rate of 0.1 was arbitrarily chosen after attempts at other rates (ex.. 0.5, 0.4) yielded disappointing results.  
